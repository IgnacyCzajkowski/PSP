...
for i, dir in enumerate(directs):
    for name in filenames:
        ...
        for line in file:
            ...
            if i % 3 == 2:
                prec_err.append(float(line.split(" ")[3]) * k_width * 0.2)
                rank_err.append(float(line.split(" ")[4]) * k_width * 0.2)
            else:
                prec_err.append(float(line.split(" ")[3]) * k_width)
                rank_err.append(float(line.split(" ")[4]) * k_width)   
            sizes.append(5)
        file.close()
    #print(str(i % 3) + " " + str(i  // 3))
        axs[i % 3, i // 3].scatter(gamma, rank_kor, sizes, label = label_dict[name], linestyle="None")
        #axs[i % 3, i // 3].errorbar(gamma, rank_kor, rank_err, ls='none')
...
for ax in axs.flat:
    ax.set(xlabel="gamma", ylabel="ranking")
    ax.set_xticks(np.arange(0, 4, step=0.5))
    ax.set_yticks(np.arange(0, 500, step=100))
    ax.grid()
for ax in axs.flat:
    ax.label_outer()
axs[0, 1].legend(loc='lower right', fontsize="5", bbox_to_anchor=(0.52, 0.5))
axs[0, 1].set_title(label="E-R")
axs[0, 0].set_title(label="B-A")
text1 = '10%'
text2 = '15%'
text3 = '20%'
axs[0, 1].text(4.33, 200, text1)
axs[1, 1].text(4.33, 200, text2)
axs[2, 1].text(4.33, 200, text3)                
plt.savefig(f"ranking_od_gammy_{sys.argv[1]}.png")